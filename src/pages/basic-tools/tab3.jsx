...
export default function Tab3({ organization }) {
  ...
  const entityName = "IOfficeEquipment";
  const fetchPlan = "iOfficeEquipment-fetch-plan";
  ...
  const columns = [
    ...
    { field: "group", headerName: "basicTools.group" },
...
function ModalUI({ hideModal, data = {} }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let data = await getFixedAssetType();
      setFixedAssets(
        data.map((current) => ({
          value: current.id,
  ...
  useEffect(() => {
    if (!data.id) return;
    setFormData({ fixedAsset: data.group.id });
  }, [data]);
    ...
    <div className="modal-content">
      ...
      <div className="modal-row">
        <FormInput
          name="group"
          select
          dataSelect={fixedAssets}
          value={formData.fixedAsset}
          label={t("basicTools.group")}
        />
        <FormInput
          name="unitOfMeasure"
          select
